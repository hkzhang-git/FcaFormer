...
class CFBlock(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6):
        ...
        self.gamma1 =  nn.Parameter(layer_scale_init_value * torch.ones((dim)),
                                  requires_grad=True).unsqueeze(-1).unsqueeze(-1) if layer_scale_init_value > 0 else None
    ...
    def forward(self, x):
        if self.use_gamma:
            x = x + self.drop_path(self.gamma1*(self.conv(x)))
            x = x + self.drop_path(self.gamma2*(self.ffn(x)))
        else:
...
class CFLayer(nn.Module):
    def __init__(self, dim_in, dim_out, input_resolution, depth, dp_rates, layer_scale_init_value=1e-6, downsample=None):
        ...
        # blocks
        self.blocks = nn.Sequential(*[
            CFBlock(dim=dim_in, drop_path=dp_rates[j],layer_scale_init_value=layer_scale_init_value)
            for j in range(depth)
        ])
...
class MBLayer(nn.Module):
    def __init__(self, dim_in, dim_out, expand_ratio, activation, input_resolution, depth, dp_rates, layer_scale_init_value=1e-6, downsample=None):
        super().__init__()
        self.dim_in = dim_in
        self.input_resolutuion = input_resolution
        self.depth=depth
        ...
        self.blocks = nn.ModuleList([
            MBBlock(dim_in, dim_in, expand_ratio, activation,
                   dp_rates[i] if isinstance(dp_rates, list) else dp_rates,
                   )
        ...
        if downsample is not None:
            self.downsample = downsample(
                input_resolution, dim=dim_in, out_dim=dim_out, activation=nn.GELU)
        else:
...
class CaBLayer(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, input_resolution, depth, num_heads,
        ...
        self.blocks = nn.ModuleList([
            CaBBlock(dim=dim_in, input_resolution=input_resolution,
                     inner_depth=i,
                     s_up=i < depth - 1,
                     num_heads=num_heads,
                     mlp_ratio=mlp_ratio,
...
#***************************************************************************************************models
# the first two stages are ConvNext blocks
class CaBViT_H(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000,
        ...
        # two deep layers:
        self.layer_2 = CaBLayer(dim_in=embed_dims[2], dim_out=embed_dims[3],
        ...
        self.layer_3 = CaBLayer(dim_in=embed_dims[3], dim_out=embed_dims[3],
...
class CaBViT_CFT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000,
        ...
        # two shallow layers:
        self.layer_0 = CFLayer(dim_in=embed_dims[0], dim_out=embed_dims[1],
                               input_resolution=(patches_resolution[0] // (2 ** 0), patches_resolution[1] // (2 ** 0)),
                               depth=depths[0],
                               dp_rates=layer_dpr[0],
                               layer_scale_init_value=1e-6,
        ...
        self.layer_1 = CFLayer(dim_in=embed_dims[1], dim_out=embed_dims[2],
                               input_resolution=(patches_resolution[0] // (2 ** 1), patches_resolution[1] // (2 ** 1)),
                               depth=depths[1],
                               dp_rates=layer_dpr[1],
                               layer_scale_init_value=1e-6,
...
class CaBViT_MBT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000,
        ...
        # two shallow layers:
        self.layer_0 = MBLayer(dim_in=embed_dims[0], dim_out=embed_dims[1], expand_ratio=4, activation=nn.GELU,
                               input_resolution=(patches_resolution[0] // (2 ** 0), patches_resolution[1] // (2 ** 0)),
                               depth=depths[0],
                               dp_rates=layer_dpr[0],
                               downsample=PatchMerging,
                               )
        self.layer_1 = MBLayer(dim_in=embed_dims[1], dim_out=embed_dims[2], expand_ratio=4, activation=nn.GELU,
                               input_resolution=(patches_resolution[0] // (2 ** 1), patches_resolution[1] // (2 ** 1)),
                               depth=depths[1],
                               dp_rates=layer_dpr[1],
                               downsample=PatchMerging,
...
@register_model
def cabvit_h_l1(pretrained=False, in_22k=False, **kwargs):
    model = CaBViT_H(embed_dims=[64, 128, 192, 320], depths=[2, 2, 6, 2], num_heads=[0, 0, 6, 10], **kwargs)
    return model

@register_model
def cabvit_h_l2(pretrained=False, in_22k=False, **kwargs):
    model = CaBViT_H(embed_dims=[96, 192, 320, 480], depths=[2, 2, 7, 2], num_heads=[0, 0, 10, 15], **kwargs)
    return model

@register_model
def cabvit_h_l3(pretrained=False, in_22k=False, **kwargs):
    # 3 12 18 3
    model = CaBViT_H(embed_dims=[96, 192, 320, 512], depths=[3, 6, 12, 3], num_heads=[0, 0, 10, 16], **kwargs)
    return model

...
def get_num_layer_stage_wise(var_name, num_max_layer):
    if var_name.startswith('backbone.patch_embed'):
        return 0
    elif 's_scale_factors' in var_name or 's_token_bias' in var_name:
        return 0
    elif var_name.startswith("backbone.layer_"):
        var_info = var_name.split('.')
        stage_id = int(var_info[1].split('_')[-1])
        op_type = var_info[2]
        if op_type=='blocks':
            return stage_id + 1
...
class LearningRateDecayOptimizerConstructor(DefaultOptimizerConstructor):
    def add_params(self, params, module, prefix='', is_dcn_module=None):
        ...
        for name, param in module.named_parameters():
            ...
            if len(param.shape) == 1 or name.endswith(".bias") or name.endswith("token_bias") or name.endswith("_biases") or 'norm' in name or name in ('pos_embed', 'cls_token') :
                group_name = "no_decay"
                this_weight_decay = 0.
            else:
                group_name = "decay"
            ...
            # print('module: {} layer_id:{} group_name:{}'.format(name, layer_id, group_name))
